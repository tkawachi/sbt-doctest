import com.github.tkawachi.doctest.{ ScaladocComment, TestGenerator, SbtDoctestInfo }
...
object DoctestPlugin extends Plugin {
  ...
  private val extract = TaskKey[Seq[(String, Seq[ScaladocComment])]]("doctest-extract")
    ...
    extract := {
      ...
      (sources in Compile).value
        ...
        .map { srcFile =>
          ...
          basename -> deserialize[Array[ScaladocComment]](tests).toList
        }
    },
    doctestGenTests := {
      val testDir = (sourceManaged in Test).value
      extract.value.flatMap {
        case (basename, tests) =>
        ...
        case ((pkg, basename), results) =>
          results.zipWithIndex.map {
            case (result, idx) =>
              ...
              val writeDir = pkg.getOrElse("").split("\\.").foldLeft(testDir) { (a: File, e: String) => new File(a, e) }
