    ...
    case leading ~ posFirstLine =>
      pythonContLines(leading).* ~ pythonExpectedLine(leading) ^^ {
        case contLines ~ ex =>
          ...
          Some(new Example(exprLines, ex, posFirstLine.pos.line))
      }
  }
    ...
    case leading ~ posFirstLine =>
      propContLine(leading).* ^^ {
        case contLines =>
          ...
          Some(new Prop(lines, posFirstLine.pos.line))
      }
  }

  def parse(input: String) = parseAll(lines, input)

  def apply(input: String): Either[String, List[DoctestComponent]] = parse(input) match {
